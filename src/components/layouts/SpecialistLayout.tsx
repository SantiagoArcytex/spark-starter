...
import { ListTodo, Users, Clock, LogOut } from "lucide-react";
...
const navItems = [
  { to: "/specialist/tasks", label: "My Tasks", icon: ListTodo, badge: "tasks" as const },
  { to: "/specialist/clients", label: "Clients", icon: Users, badge: null },
  { to: "/specialist/time-logs", label: "Time Logs", icon: Clock, badge: null },
];
    ...
    <div className="flex min-h-screen bg-background">
      <aside className="hidden w-64 flex-col border-r border-sidebar-border bg-sidebar md:flex">
        ...
        <nav className="flex-1 space-y-1 px-3 py-4">
          {navItems.map((item) => {
            const badgeCount = item.badge === "tasks" ? counts.pendingApprovals : 0;
            return (
              <NavLink
                key={item.to}
                to={item.to}
        ...
        <div className="border-t border-sidebar-border p-4">
          <div className="mb-3 text-sm">
            <p className="font-medium text-foreground truncate">{profile?.full_name || "Specialist"}</p>
