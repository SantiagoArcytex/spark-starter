...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { email, full_name, company, monthly_allocation, rate } = await req.json();
    ...
    // Insert profile, role, and client_account
    await Promise.all([
      supabaseAdmin.from("profiles").upsert({ user_id: userId, email, full_name, company: company || null }),
      supabaseAdmin.from("user_roles").insert({ user_id: userId, role: "client" }),
      supabaseAdmin.from("client_accounts").insert({
        user_id: userId,
        monthly_allocation: monthly_allocation || 10,
        current_rate: rate || 100,
        hours_purchased: monthly_allocation || 10,
      }),
    ]);
